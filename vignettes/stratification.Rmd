...
`strat_breaks()` stratifies data based on user-defined breaks in `mraster`. This algorithm is vectorized. The user can provide an `mraster` with as many layers as they wish as long as the `breaks` paramters is list of equal length comprised of numeric vectors. 
...
```{r,warning=F,message=F}
#--- stratify on 1 metric only ---#
strat_breaks(mraster = mraster[[1:2]],
