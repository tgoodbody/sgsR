...
`sraster` are derived from `strat_*` algorithms (e.g. `strat_quantiles()`). This function used the distribution of `mraster$zq90` and stratified data into 4 equally sized strata.
...
The `sraster` output then becomes an input parameter for the `sample_strat()` algorithm.
