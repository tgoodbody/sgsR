...
The location of sample units can also be adjusted using the `locations` parameter, where `centers` takes the center, `corners` takes all corners, and `random` takes a random location within each tessellation.
...
This algorithm uses moving window (`wrow` and `wcol` parameters) to filter the input `sraster` to prioritize sample units allocation to where stratum pixels are spatially grouped, rather than dispersed individuals across the landscape.
...
* **Rule 2** - If no additional sample units exist to satisfy desired sample size, individual stratum pixels are sampled.

The rule applied to a select a particular sample units is defined in the `rule` attribute of output samples. We give a few examples below:
...
As seen on the code in the example above, the defined `mindist` parameter specifies the minimum euclidean distance that sample units must be apart from one another.
...
sample_clhs(mraster = mraster, # input
            ...
            access = access, # define access road network
            buff_inner = 100, # inner buffer - no sample units within this distance from road
...
First we can create an `existing` sample for our example. Lets imagine we have a systematically sampled dataset of ~900 samples, and we know we only have resources to sample 300 of them. We have some ALS data available (`mraster`), which we will use as our distributions to sample within.
...
We see from the output that we get 300 sample units that are a sub-sample of the original `existing` sample. The plotted output shows cumulative frequency distributions of the population (all `existing` samples) and the sub-sample (the 300 samples we requested).
