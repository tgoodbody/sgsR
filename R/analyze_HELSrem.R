...
analyze_HELSrem <- function(mraster = NULL,
  ...
  ggplot(kk, aes(x = x, y = y)) +
    geom_tile(aes(fill = z), colour = "grey50") +
    scale_fill_continuous_divergingx(palette = "RdBu", mid = 0.7) +
    facet_grid(. ~ type)


  p1 <- ggplot(k, aes(x = x, y = y, fill = z)) +
    geom_tile(colour = "grey50") +
    scale_fill_viridis() +
    theme_light()
  p2 <- ggplot(k1, aes(x = x, y = y, fill = z)) +
    geom_tile(colour = "grey50") +
    scale_fill_viridis() +
    theme_light()
  p3 <- ggplot(k2, aes(x = x, y = y, fill = z)) +
    geom_tile(colour = "grey50") +
    scale_fill_viridis() +
    theme_light()


  ggplot(k, aes(x = x, y = y, fill = z)) +
    geom_tile(colour = "grey50") +
    scale_colour_gradientn(colours = c("red", "white", "darkblue"), values = c(min(k$z), median(k$z), max(k$z)))
